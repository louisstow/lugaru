...
void Objects::Sort()
{
	...
	for (i = 0; i < numobjects-1; i++) {
		...
		for (j = i+1; j < numobjects; j++) {
			d1 = findDistancefastflat(&viewer,&position[render_order[jMin]]);
			d2 = findDistancefastflat(&viewer,&position[render_order[j]]);
			
			// fprintf(stderr, "j: %d jMin: %d %f > %f\n", j, jMin, d2, d1);
			if (d2 > d1) {
				jMin = j;
			}
		}
		...
		if (jMin != i) {
			tmp = render_order[i];
			render_order[i] = render_order[jMin];
			render_order[jMin] = tmp;
		}
	}
							...
							glPushMatrix();
								...
								if(type[i]==treeleavestype){
									...
									if(!hidden){
										...
										if(distance<1)glAlphaFunc(GL_GREATER, 0.5);
									}
									if(hidden){
										glDepthMask(0);
										glEnable(GL_BLEND);
